...
function renderFunnelFin() {
  ...
  function emptyBucket() {
    return {ho:0,ho_s:0,ho_t:0, cnh:0,cnh_s:0,cnh_t:0,
            app:0,app_s:0,app_t:0, cna:0,cna_s:0,cna_t:0};
  }
  function addRow(bucket, r) {
    bucket.ho   += r.ho_total||0;         bucket.ho_s  += r.ho_sales||0;         bucket.ho_t  += r.ho_ti||0;
    bucket.cnh  += r.can_no_ho_total||0;  bucket.cnh_s += r.can_no_ho_sales||0;  bucket.cnh_t += r.can_no_ho_ti||0;
    bucket.app  += r.app_total||0;        bucket.app_s += r.app_sales||0;        bucket.app_t += r.app_ti||0;
    bucket.cna  += r.can_no_app_total||0; bucket.cna_s += r.can_no_app_sales||0; bucket.cna_t += r.can_no_app_ti||0;
  }
  ...
  const kpiList = [
    { id:'ho',
      cur: crFn(cur.ho,  cur.ho  + cur.cnh),  prv: crFn(prv.ho,  prv.ho  + prv.cnh),
      sub: `Sales: ${fmtPct(crFn(cur.ho_s, cur.ho_s + cur.cnh_s))} · TI: ${fmtPct(crFn(cur.ho_t, cur.ho_t + cur.cnh_t))}` },
    { id:'app',
      cur: crFn(cur.app, cur.app + cur.cna),  prv: crFn(prv.app, prv.app + prv.cna),
      sub: `Sales: ${fmtPct(crFn(cur.app_s, cur.app_s + cur.cna_s))} · TI: ${fmtPct(crFn(cur.app_t, cur.app_t + cur.cna_t))}` },
    { id:'vta',
  ...
  const chartDefs = [
    { chartKey:'funnel-ho', map: timeMap, sets:[
        {label:'Total',  fn: m => crFn(m.ho,   m.ho   + m.cnh),   col:'#1a3a5c'},
        {label:'Sales',  fn: m => crFn(m.ho_s, m.ho_s + m.cnh_s), col:'#2980b9', dash:[4,3]},
        {label:'Con TI', fn: m => crFn(m.ho_t, m.ho_t + m.cnh_t), col:'#e67e22', dash:[2,3]},
    ]},
    { chartKey:'funnel-app', map: timeMap, sets:[
        {label:'Total',  fn: m => crFn(m.app,   m.app   + m.cna),   col:'#1a3a5c'},
        {label:'Sales',  fn: m => crFn(m.app_s, m.app_s + m.cna_s), col:'#2980b9', dash:[4,3]},
        {label:'Con TI', fn: m => crFn(m.app_t, m.app_t + m.cna_t), col:'#e67e22', dash:[2,3]},
    ]},
